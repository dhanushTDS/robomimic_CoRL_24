...
import cv2
from robomimic.utils.gaze_socket_client import SimpleClient  # Gaze Sensor Integration
...
from robomimic.algo import RolloutPolicy, RolloutPolicy_LIRA  # TODO: one of the main changes
# from screeninfo import get_monitors
from robomimic.utils.rollout_utils import *  # NOTE(DHANUSH) : Util functions


def rollout(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False, camera_names=None,
            rollout_number=None, socket_obj=None):
    """
    ...
    assert isinstance(policy, RolloutPolicy_LIRA)
    # assert not (render and (video_writer is not None)) #NOTE(dhanush): You have commneted this
    ...
    camera_transformation_matrix = env.get_camera_transform_matrix(camera_names[0], 2160,
                                                                   3840)  # NOTE(dhanush) : Need to adapt the number as per screen size
    ...
    subgoal_stats = dict(var_eef_world_coordinates=[],
                         var_eef_pixel_locations=[])  # STORING VARIANCE, NOT ACTUAL VALUES
    ...
    try:
        for step_i in range(0, horizon):  # TODO: Check the indexing logic
            ...
            if step_i % interval == 0:
                # SUBGOAL SAMPLES FROM {VAE} - HIGH LEVEL PLANNER
                sg_proposals = policy.get_subgoal_proposals_LIRA(ob=obs)

                world_points = subgoal_ee_pos(
                ...
                var_world_sg, var_pixel_sg = calculate_max_column_variance(world_points), calculate_max_column_variance(
                    pp_sgs)  # Computing Column wise variance and taking the max

                # TRACKING THE VARIANCE IN BOTH WOLRD POS AND {RENDER_SCREEN_FRAME}
                ...
                subgoal_index_from_gaze = gaze_subgoal_index(pp_sgs=pp_sgs,
                                                             gaze_input=gaze_input)  # CLOSEST SUBGOAL TO GAZE IN PIXEL SPACE
            ...
            # visualization
            if render:
                env.render(mode="human", camera_name=camera_names[0], width=3840,
                           height=2160)  # TODO: PREVENT HARDCODING THE SCREEN RESOLUTION

            if video_writer is not None:
                if video_count % video_skip == 0:
                    video_img = []
                    for cam_name in camera_names:
                        current_frame = env.render(mode="dual", height=2160, width=3840,
                                                   camera_name=cam_name)  # TODO: check dual render in env_robosuite.py
                        ...
                        # NOTE(dhanush) : MARKING CHOOSEN/ACTIVE SUBGOAL {EEF POS} ON VIDEO
                        edited_frame = cv2.drawMarker(np.uint8(edited_frame.copy()),
                                                      (int(pp_sgs[subgoal_index_from_gaze][0]),
                        ...
                        if step_i % interval == 0:
                            ...
                            # NOTE(dhanush) : MARKING ALL SUBGOAL SAMPLES FROM THE PLANNER ON VIDEO
                            for point in pp_sgs:
                        ...
                        for i, text in enumerate(texts):
                            position = (edited_frame.shape[1] - max_text_width - 10, start_y + i * max_text_height)
...
def run_trained_agent(args):
    ...
    # assert not (args.render and write_video) # either on-screen or video but not both # NOTE(dhanush) : You have commented this
    if args.render:
    ...
    gaze_client = SimpleClient('192.168.1.93', 5478, 102)  # NOTE(dhanush): IPv4(windows), PORT NUMBER, len(meesage)
    # gaze_client.connect_to_server()  # CONNECT TO CLIENT
    ...
    for i in range(rollout_num_episodes):
        stats, traj, subgoal_stats = rollout(
            ...
            camera_names=args.camera_names, rollout_number=i, socket_obj=gaze_client
    ...
    # gaze_client.disconnect()  # DISCONNECT GAZE SENSOR

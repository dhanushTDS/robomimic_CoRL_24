...
def extract_trajectory(
        ...
        states,
        actions,
        gazes,  # NOTE(dhanush): we are now making gazes also part of the function arguement
        done_mode,
        camera_names=None,
        camera_height=84,
        camera_width=84,
    ...
    obs.update({"human_gaze": gazes[0]})  # NOTE(dhanush): to the intial observation, we are adding gaze[0]
    ...
    traj = dict(
        ...
        gazes=np.array(gazes),  # NOTE(dhanush): we are now making gazes also part of the trajectory
    ...
    for t in range(1, traj_len + 1):
        ...
        if t == traj_len:
            ...
            next_obs.update({"human_gaze": gazes[-1]})  # NOTE(dhanush): final gaze for the final action

        else:
            # reset to simulator state to get observation
            next_obs = env.reset_to({"states": states[t]})
            next_obs.update({"human_gaze": gazes[t]})  # NOTE(dhanush):  gaze being added to the next observation
...
def get_camera_info(
    ...
    for cam_name in camera_names:
        K = env.get_camera_intrinsic_matrix(camera_name=cam_name, camera_height=camera_height,
                                            camera_width=camera_width)
...
def dataset_states_to_obs(args):
    ...
    for ind in tqdm(range(len(demos))):
        ...
        # extract obs, rewards, dones , gaze
        # NOTE(dhanush) : modifying to also extract gaze now
        actions = f["data/{}/actions".format(ep)][()]
        gazes = f["data/{}/gazes".format(ep)][()]  # NOTE(dhanush) : gaze being extracted
        # NOTE(dhanush): use the modified extract_trajectory function to now also use gaze
        ...
        ep_data_grp.create_dataset("gazes",
                                   data=np.array(traj["gazes"]))  # NOTE(dhanush) : gaze being added to ep data grp
        # TODO(dhanush): check if gazes needed to be added anywhere after this part of the fucntion
